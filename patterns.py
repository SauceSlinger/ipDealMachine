...
PATTERNS = {
    'number_of_units': [
        ...
        re.compile(r'(\d+)-plex', re.IGNORECASE)  # Catches "4-plex"
    ],
    'monthly_rent_per_unit': [
        re.compile(r'rent\s*per\s*unit\s*[:=]\s*\$?([\d,.]+)', re.IGNORECASE),  # Added '.' for decimals
    ...
    'property_taxes': [
        ...
        re.compile(r'tax\s*expense\s*[:=]\s*\$?([\d,.]+)', re.IGNORECASE)  # Catches "Tax Expense: $6,659"
    ],
    'insurance': [
    ...
    'purchase_price': [
        ...
        re.compile(r'list\s*price\s*[:=]\s*\$?([\d,.]+)', re.IGNORECASE),  # Already captures "$1,000,000"
    ...
    'gross_scheduled_income': [  # New field to capture "Gross Scheduled Inc: $67,200"
        re.compile(r'gross\s*scheduled\s*inc\s*[:=]\s*\$?([\d,.]+)', re.IGNORECASE),
        re.compile(r'gross\s*income\s*[:=]\s*\$?([\d,.]+)', re.IGNORECASE)
    ]
}
...
def extract_data_with_patterns(text):
    ...
    # Clean text for better matching - remove all whitespace and convert to lower
    # Note: For some patterns, preserving some whitespace (e.g., between "Tax" and "Expense")
    # might be beneficial. However, the current regexes are robust enough with `\s*`.
